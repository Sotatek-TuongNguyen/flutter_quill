...
class QuillController extends ChangeNotifier {
  ...
  void updateComposing(TextRange comp) {
    final underlineStyles = toggledStyle.attributes.values.where(
      (s) => s.key == Attribute.underline.key && s.value == true,
    );
    if(comp != TextRange.empty) {
      if(underlineStyles.isEmpty) {
        document.format(comp.start, comp.end - comp.start, Attribute.underline);
        formatSelection(Attribute.clone(Attribute.underline, null));
      }
    } else {
      if(_composing != TextRange.empty) {
        if(underlineStyles.isEmpty) {
          document.format(_composing.start, _composing.end - _composing.start, Attribute.clone(Attribute.underline, null));
        }
      }
    }
    _composing = comp;
  ...
  void updateSelection(TextSelection textSelection, ChangeSource source) {
    if(_composing != TextRange.empty) {
      updateComposing(TextRange.empty);
    }
